    ...
    : QMainWindow(parent)
{
    ...
    rec->setPixmap(pix.scaled(rec->width(),rec->height()));
    ml->addWidget(rec);
...
void MainWindow::showslot()
{
    ...
    for(int i=0;i<tab->rowCount();i++)
    {
        ...
        connect(bsl,SIGNAL(clicked(bool)),this,SLOT(averagesalesslot()));
        tab->setCellWidget(i,2,bsl);
        QPushButton *shb=new QPushButton;
        shb->setText("SHOW");
        shb->setProperty("show",deps[i].get_dep_name());
        tab->setCellWidget(i,3,shb);
        connect(shb,SIGNAL(clicked(bool)),this,SLOT(showempsslot()));
    }
    series->clear();
   for(int i=0;i<deps.size();i++)
   {
...
void MainWindow::averagesal()
{
    int pos=-1;
    QPushButton *bpro=(QPushButton *)sender();
    QString search=bpro->property("sal").toString();
    for(int i=0;i<deps.size();i++)
    {
        if(deps[i].get_dep_name()==search)
        {
            pos=i;
        }
    }
    QMessageBox::information(this,"INFO","<p style=color:blue>AVERAGE SALARY OF DEPARTMENT:"+deps[pos].get_dep_name()+":"+QString::number(deps[pos].average_salary())+"</p>");
}

void MainWindow::averagesalesslot()
{
    int pos=-1;
    QPushButton *bprop=(QPushButton *)sender();
    QString search=bprop->property("sales").toString();
    for(int i=0;i<deps.size();i++)
    {
        if(deps[i].get_dep_name()==search)
        {
    ...
    QMessageBox::information(this,"INFO","<p style=color:blue>AVERAGE SALES OF DEPARTMENT:"+deps[pos].get_dep_name()+":"+QString::number(deps[pos].average_sales())+"</p>");
}

void MainWindow::showempsslot()
{
    int pos=-1;
    QPushButton *bprop=(QPushButton *)sender();
    QString search=bprop->property("sales").toString();
    ...
    QMessageBox::information(this,"INFO","<p style=color:blue>DEPARTMENT_INFO::\n"+deps[pos].to_string_md()+"</p>");
